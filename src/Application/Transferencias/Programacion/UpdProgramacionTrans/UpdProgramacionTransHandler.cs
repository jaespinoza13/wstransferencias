...
public class UpdProgramacionTransHandler : RequestHandler<ReqUpdProgramacionTrans, ResUpdProgramacionTrans>

{
    ...
    private const string str_operacion = "UPD_PROGRAMACION_TRANS";



    public UpdProgramacionTransHandler(IOptionsMonitor<ApiSettings> option, ILogs logs, IProgramacionDat programacionDat, IWsOtp wsOtp, IWsAlfresco wsAlfresco)
    {
    ...
    protected override ResUpdProgramacionTrans Handle(ReqUpdProgramacionTrans reqUpdProgramacionTrans)
    {
        ResUpdProgramacionTrans respuesta = new();
        respuesta.LlenarResHeader( reqUpdProgramacionTrans );
        var res_tran = new RespuestaTransaccion();
        _logsService.SaveHeaderLogs( reqUpdProgramacionTrans, str_operacion, MethodBase.GetCurrentMethod()!.Name, str_clase );

        try
        {
            var requiereOtp = _wsOtp.ValidaRequiereOtp( reqUpdProgramacionTrans, "UPD_PROGRAMACION_TRANS" ).Result;

            var ejecutarSolicitud = false;
            if (requiereOtp)
            {
                var res_tran_otp = _wsOtp.ValidaOtp( reqUpdProgramacionTrans ).Result;
            ...
            if (ejecutarSolicitud) {
                var req_get_contrato = JsonSerializer.Deserialize<ReqGetContratoPT>( JsonSerializer.Serialize( reqUpdProgramacionTrans ) )!;
                var req_add_contrato_alfresco = JsonSerializer.Deserialize<DocumentoAlfresco>( JsonSerializer.Serialize( reqUpdProgramacionTrans ) )!;
                ...
                {
                    Functions.llenar_datos_alfresco( req_add_contrato_alfresco, reqUpdProgramacionTrans );
                    ...
                    req_add_contrato_alfresco.str_subtipo_documento = reqUpdProgramacionTrans.str_nemonico_canal;
                    ...
                    else
                    {
                        ...
                        reqUpdProgramacionTrans.str_id_contrato = str_id_alfresco;
                        res_tran = _programacionDat.UpdProgramacionTrans( reqUpdProgramacionTrans );
